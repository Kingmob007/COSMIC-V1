...
if (conf.LANG == 'EN') {
    ...
    already_on = 'COSMICV1 chatbot is already on.'
    already_off = 'COSMICV1 chatbot is currently turned off!.'
    succ_on = 'COSMICV1 chatbot on! Restarting to make chatbot ✅'
    succ_off = 'COSMICV1 chatbot off Restarting to make normal  ❤️'
}
if (conf.LANG == 'ML') {
    fulleva_dsc = 'പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായ COSMICV1 chatbot സജീവമാക്കുന്നു. നിങ്ങളുടെ അക്കൗണ്ട് ഒരു ചാറ്റ്ബോട്ടാക്കി മാറ്റുക!'
    already_on = 'കൃത്രിമബുദ്ധി ഇതിനകം പൂർണ്ണമായി പ്രവർത്തിക്കുന്നു.'
    already_off = 'AI നിലവിൽ സെമി-ഫംഗ്ഷണൽ ആണ്.'
    succ_on = 'COSMICV1 പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായി തുറന്നു! കുറച്ച് കാത്തിരിക്കൂ! ✅'
    succ_off = 'COSMICV1 സെമി-ഫങ്ഷണൽ ആയി സജ്ജമാക്കുക! കുറച്ച് കാത്തിരിക്കൂ! ☑️'
}

New.addCommand({ pattern: 'khadher ?(.*)', desc: _dsc, fromMe: true,dontAddCommandList: true, usage: '.khadher on / off' }, (async (message, match) => {
